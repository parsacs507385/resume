...
class NimAI():
    ...
    def choose_action(self, state, epsilon=True):
        ...
        if (epsilon) and (random.random() <= self.epsilon):
            return random.sample(sorted(availableActions), 1)

        best = float("-inf")

        for availableAction in availableActions:
            if best < self.get_q_value(state, availableAction):
                best = self.get_q_value(state, availableAction)
                bestAct = availableAction
